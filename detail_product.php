...
<head>
    ...
    <title><?= htmlspecialchars($selected['name']) ?> — UMKMTerdekat</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                ...
                <nav id="menu" class="hidden lg:flex items-center space-x-8">
                    <a href="index.php" class="text-gray-700 hover:text-green-600 transition font-medium">Beranda</a>
            ...
            <div id="mobile-menu"
                ...
                <a href="index.php" class="block text-gray-700 hover:text-green-600 font-medium">Beranda</a>
    ...
    <main class="py-12">
        <div class="max-w-6xl mx-auto px-6">
            <nav class="text-sm text-gray-500 mb-6">
                <a href="index.php" class="hover:text-green-600">Beranda</a> <a href="#produk" class="hover:text-green-600">Produk</a> <span class="text-gray-800"><?= htmlspecialchars($selected['name']) ?></span>
            </nav>
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 items-start">
                    ...
                    <div class="md:col-span-1 flex items-center justify-center">
                        <div class="w-full max-w-md bg-gradient-to-br from-green-50 to-green-100 rounded-2xl p-8 text-center">
                            <div class="text-[8rem] mb-4"><?= htmlspecialchars($selected['image']) ?></div>
                            <div class="text-sm text-gray-500">Penjual</div>
                            <div class="font-semibold text-lg text-gray-900"><?= htmlspecialchars($selected['seller']) ?></div>
                        </div>
                    </div>

                    <!-- Info -->
                    <div class="md:col-span-2">
                        <h1 class="text-2xl md:text-3xl font-bold text-gray-900 mb-2"><?= htmlspecialchars($selected['name']) ?></h1>
                        <div class="flex items-center justify-between md:justify-start gap-6 mb-4">
                            <div class="text-2xl font-bold text-green-600"><?= htmlspecialchars($selected['price']) ?></div>
                            <div class="text-sm text-gray-500">Stok: <span class="font-semibold text-gray-700"><?= (int)$selected['stock'] ?></span></div>
                        </div>

                        <p class="text-gray-700 mb-6"><?= htmlspecialchars($selected['description']) ?></p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                            ...
                            <div>
                                ...
                                ?>
                                <a href="<?= htmlspecialchars($businessUrl) ?>" class="inline-flex items-center gap-3 w-full sm:w-auto justify-center border-2 border-green-600 text-green-600 hover:bg-green-50 px-4 py-2 rounded-xl text-sm font-semibold transition">
                                    <i class="fa-solid fa-store"></i>
                                    <div class="text-left">
                                        <div class="font-semibold"><?= htmlspecialchars($selected['seller']) ?></div>
                                        <div class="text-xs text-gray-500"><?= htmlspecialchars($selected['address']) ?></div>
                                    </div>
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            ...
                            ?>
                            <a href="<?= htmlspecialchars($mapsLink) ?>" target="_blank" rel="noopener noreferrer" class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-xl text-sm font-semibold transition shadow-md flex items-center gap-2">
                                <i class="fa-solid fa-location-dot"></i> Lihat Rute Ke Penjual
                            </a>

                            <a href="<?= htmlspecialchars($waLink) ?>" target="_blank" rel="noopener noreferrer" class="bg-white border-2 border-green-600 text-green-600 hover:bg-green-50 px-6 py-3 rounded-xl text-sm font-semibold transition flex items-center gap-2">
                                <i class="fa-brands fa-whatsapp"></i> Hubungi Penjual via WhatsApp
                            </a>
                        </div>

                        <?php if ($selected['stock'] <= 0): ?>
                            <div class="mt-4 text-sm text-red-600">Stok habis — silakan hubungi penjual untuk ketersediaan.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <section id="produk" class="mt-12">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="index.php#produk" class="text-green-600 hover:underline text-sm">Lihat Semua</a>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    <?php foreach ($products as $p): if ($p['id'] === $selectedId) continue; ?>
                        <a href="?id=<?= htmlspecialchars($p['id']) ?>" class="block bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all overflow-hidden border border-gray-100 hover:border-green-200 transform hover:-translate-y-1">
                            ...
                            <div class="p-4">
                                <div class="text-xs text-gray-500 mb-1"><?= htmlspecialchars($p['seller']) ?></div>
                                <h4 class="font-semibold text-gray-900 mb-1"><?= htmlspecialchars($p['name']) ?></h4>
                                <div class="text-green-600 font-bold"><?= htmlspecialchars($p['price']) ?></div>
                            </div>
                        </a>
                    <?php endforeach; ?>
                </div>
        ...
        <div class="max-w-7xl mx-auto px-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-8">
                <div>
                    ...
                    <p class="text-green-100 leading-relaxed text-sm">Platform terpercaya untuk menemukan dan mendukung UMKM lokal di seluruh Indonesia</p>
                </div>
            ...
            <div class="border-t border-green-500 pt-6 text-center text-green-100 text-sm">
                &copy; <?= date('Y') ?> <strong>UMKMTerdekat</strong>. Semua Hak Dilindungi.
            </div>
        </div>
    </footer>
